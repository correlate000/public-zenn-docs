...
flowchart TD
    ...
    subgraph Layer2["🗄️ 蓄める層"]
        B[BigQuery<br/>データウェアハウス]
        B --> |"18テーブル"|BB1["工数ログ"]
        B --> |"SQL分析"|BB2["案件データ"]
        B --> |"構造化保存"|BB3["KPI・請求書"]
    end

    subgraph Layer3["👁️ 見る層"]
        D[Discord<br/>ダッシュボード]
        D --> |"朝会Bot"|DB1["今日やること"]
        D --> |"スラッシュコマンド"|DB2["売上推移"]
        D --> |"スマホアクセス"|DB3["経営状況一目"]
    end
...
graph LR
    subgraph Sources["📊 データソース"]
        ...
        S2[freee API]
        S3[GAS]
        S4["Obsidian<br/>(Markdown)"]
        S5[Drive API]
        S6[Google Ads API]
    end

    subgraph Pipeline["⚙️ 統合基盤"]
        CR["Cloud Run API<br/>(FastAPI)"]
        BQ["BigQuery<br/>(18テーブル)"]
        CS["Cloud Scheduler<br/>(日次バッチ)"]
    end

    subgraph Outputs["📱 配信先"]
        D1["Discord Bot<br/>(朝会)"]
        D2["Discord<br/>(スラッシュコマンド)"]
        D3["Discord<br/>(アラート)"]
    end
    ...
    style CR fill:#90caf9
    style BQ fill:#fff59d
