...
記事の最後では、月額$279 → $127 への削減計算例も示します。
...
|--------|----------|----------|-------------|
| claude-haiku-4-5 | $0.80 | $4.00 | 分類・要約・簡易 QA・変換 |
...
1,000 件のリクエスト（各入力 500 トークン・出力 100 トークン）を claude-haiku-4-5 で処理する場合:
...
Batches + キャッシュ + Haiku 振り分けを組み合わせた場合の変化を見ます。
...
施策2: シンプルな FAQ 応答（全体の 60%）を claude-haiku-4-5 に振り分け
- Haiku 出力: 10,800 件 × 400 tokens × ($4.00 / 1,000,000) = $17.28
- Sonnet 出力: 7,200 件 × 400 tokens × ($15.00 / 1,000,000) = $43.20
- 削減: $108.00（出力） → $60.48（約 44% 削減）
...
| サポート出力 | $108.00 | $60.48 | $47.52 |
| レポート処理 | $63.00 | $31.50 | $31.50 |
| 合計 | $279.00 | $127.08 | $151.92 |

3 つの施策を組み合わせることで、月額コストを約 54% 削減できます。
