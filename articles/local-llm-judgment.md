...
#### MoEアーキテクチャと「実効パラメータ数」の読み方（2026年4月補足）

2026年4月時点で、主要なローカルLLMの多くがMoE（Mixture of Experts）アーキテクチャを採用しています。MoEでは、推論時に使われる **活性パラメータ** と、モデル全体の **総パラメータ** が大きく異なります。

| モデル | 活性パラメータ | 総パラメータ | ライセンス |
|:--|:--|:--|:--|
| Llama 4 Maverick | 17B | 400B | Llama |
| Llama 4 Scout | 17B | 109B | Llama |
| Qwen3-Coder-Next | 3B | 80B | Apache 2.0 |
| LLM-jp-4 32B-A3B | 3.8B | 32B | Apache 2.0 |
| Gemma 4 | -（MoE採用） | - | Apache 2.0 |

「400Bモデルでも、必要メモリは活性パラメータ17B相当」というのがMoEの特性です。スペック表の数字をそのまま比較するのは正確ではなく、活性パラメータ数と必要メモリを確認するのが実態に近い判断になります。

#### 超軽量1ビットLLM: Bonsai-8B（2026年3月登場）

PrismMLが2026年3月31日に公開した **Bonsai-8B** は、1ビット量子化で8Bモデルをわずか**1.15GB**に圧縮したモデルです。

- **GPU不要**: Apple Silicon MacBook（8GB RAM）で動作確認済み
- **iPhone 17 Proで40 tokens/s**の推論速度
- モデルサイズが1GB台のため、オフラインアプリへの組み込みも現実的

従来「GPU必須」と言われていた8B規模のモデルが、メモリ8GBの端末でも実用的に動くようになっています。MoEと1ビット量子化の普及により、**「パラメータ数＝性能」「大きいモデル＝高スペック必須」という前提は既に古くなっています**。
...
https://zenn.dev/correlate_dev/articles/morning-bot-ai
