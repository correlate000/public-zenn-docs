...
私自身、ISVDサイトの記事制作セッションとクライアント案件のセッションを同時に走らせていたとき、この問題に遭遇しました。コンテキスト圧縮が入った直後、クライアント案件のセッションがISVD側のセッション記録ファイルに作業ログを書き込み始めたのです。2つのセッション記録の混在で判別不能となり、手動分離に30分以上。セッション記録の信頼性が根底から揺らぐ経験です。
