...
署名検証は必須です。HMAC-SHA256で署名を検証し、比較には `hmac.compare_digest()` を使う。不正なリクエストには 400 を返してfreeeに再送させないことが重要です。

冪等性は `event_id` によるMERGEで確保します。ネットワーク障害やfreeeのリトライで同じイベントが複数回届いても、BigQueryに重複レコードが積まれることはありません。

5秒以内のレスポンスを守ります。freeeの仕様でタイムアウトになるとリトライが発生。重い処理はCloud Tasksに委譲してWebhookエンドポイントは素早くレスポンスを返す設計にします。
