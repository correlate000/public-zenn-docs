...
「大きいPR」はレビュー品質を劣化させます。変更行数が500行を超えると、レビュアーの集中力は目に見えて低下し、見落としが増えます。しかし、PRを小さく保つ文化を根付かせることは、技術的な問題ではなく組織文化の問題であり、短期間では解決できません。
...
シングルエージェントでも高品質なレビューは可能です。しかし、**専門特化**することで各エージェントはより深い観点でコードを分析できます。SecurityエージェントにはOWASPの知識を注入し、PerformanceエージェントにはN+1クエリパターンの辞書を持たせる、といった設計が実現できます。
