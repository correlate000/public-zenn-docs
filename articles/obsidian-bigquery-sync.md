---
title: "Obsidian→BigQueryナレッジ同期 — 1人法人の第二の脳をDB化する"
emoji: "📊"
type: "tech"
...
Python Client Libraryを使うと、サービスアカウントキーの管理やADCの設定が必要になります。一方、`bq` CLIなら `gcloud auth login` さえ済んでいれば、追加の認証設定なしで動作します。1人法人の開発マシンで日次実行するスクリプトとして、この「設定の簡潔さ」は大きなメリットだった。
