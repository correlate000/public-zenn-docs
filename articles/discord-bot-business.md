...
実装のポイントは、**Embedフィールドの`inline`パラメータ**の使い分けです。
...
**対策**: discord.pyの自動再接続機能（デフォルト有効）に任せつつ、`bot.is_ready()`チェックを挟んで未接続時はHTTP 503を返す設計に。Cloud Schedulerのリトライ設定（最大3回、バックオフ30〜300秒）でカバーしています。
