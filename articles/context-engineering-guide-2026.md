...
「プロンプトエンジニアリング」という言葉が定着して久しいですが、2025年後半からAI界隈で急速に注目を集めている概念があります。**コンテキストエンジニアリング（Context Engineering）**です。
...
Karpathyが示したメンタルモデルが直感的に理解しやすいです。**LLMをCPU、コンテキストウィンドウをRAM（ワーキングメモリ）**として捉えるのです。CPUにどのデータをRAMに乗せるかを設計するのがコンテキストエンジニアリングの本質です。
