...
型ヒントは、そのギャップを静的解析の段階で埋めるための仕組みです。Python 3.5でPEP 484が導入されてから10年近くが経ち、型ヒントのエコシステムは劇的に成熟。mypy と pyright という2つの強力な型チェッカーの登場、FastAPI や Pydantic のような型ヒントを中核に据えたフレームワークの普及がその証拠です。
...
mypy と pre-commit の相性に関して注意が必要です。pre-commit は hooks を隔離された仮想環境で実行するため、プロジェクトの依存関係が非可視。`additional_dependencies` へのスタブパッケージ・型付きライブラリの列挙が解決策です。
