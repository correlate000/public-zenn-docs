...
通常ビューはシンプルで管理コストがゼロですが、高頻度クエリではコストが膨大になります。マテリアライズドビューはBigQueryが自動管理し、`max_staleness` の設定でコストとデータ鮮度のバランス調整が可能。集計テーブルは複雑な集計ロジックが必要な場合の最終手段で、冪等なMERGEで安全に更新できます。
