...
Cloud Run上で動いているFastAPIアプリがあります。Google Ads API、freee API、GAS連携、BigQueryクエリ、Discord Bot――業務に必要な機能を片っ端から`main.py`に書き足していった結果、ファイルは**約3500行**に膨れ上がっていました。
...
既存コードを大規模にリファクタリングする余裕はありません。ただ、新機能だけでも別モジュールにすれば、少なくとも**これ以上の肥大化は止められる**。
...
main.py自体の行数は大きく減ってはいません。今回分離したのは新規コードであり、既存コードには手を入れていないからです。しかし、**これ以上の肥大化に歯止めをかけた**ことと、**次の分割の前例を作った**ことに意味があります。
