...
この記事は「申請が通ったけど、次に何をすればいいかわからない」という方を主な読者として想定しています。コードサンプルを含む実践的な内容のため、技術担当者と広告担当者が一緒に読み進めるのが理想的。各コードブロックには、広告担当者向けの役割説明も添付済みです。
...
Ad Grantsのポリシーでは、少なくとも1つの「意味のあるコンバージョン」が設定されていることが求められます。ここでいうコンバージョンとは、問い合わせフォームの送信、寄付の完了、ボランティア登録など、組織の目標に直結するアクション。GA4との連携が、この設定を正確に行うための基盤になります。
