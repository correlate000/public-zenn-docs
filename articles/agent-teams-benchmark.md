...
emoji: "⚡"
type: "tech"
topics: ["claudecode", "ai", "agentteams", "devproductivity", "parallelprocessing"]
published: false
...
注目すべきは「4エージェントで3倍速」という非線形な関係です。単純な並列化であれば4倍近い速度向上が期待できますが、実際には3倍にとどまっている。この差分にはエージェント間の調整コスト、ファイルアクセスの競合回避、オーケストレーション層のオーバーヘッドが含まれていると考えられます。
...
実際のコーディングタスクでも事情は同じです。依存関係のある複数ファイルを編集する場合、ある変更が完了するまで次の変更に着手できないケースが発生する——4エージェントを動かしていても、実質的に稼働しているのは常に4つとは限りません。

それでも「3倍速」という数字は実用上、無視できない意味を持ちます。18分かかっていた作業が6分で完了すれば、開発者の集中が途切れる前に結果を得られる。認知的なコストという観点からも、このレイテンシ削減の効果は大きいと言えます。
...
**Swarms** は、多数のエージェントがフラットな関係で協調するモデルです。中央オーケストレーターを持たず、エージェント同士が直接通信する構造——スケーラビリティに優れる一方、調整コストが分散する特徴があります。

**Agent Teams（Claude Code）** は、オーケストレーター＋サブエージェントという階層構造です。中央のオーケストレーターがタスクを分割し、各エージェントに割り当て、結果を統合する。制御が集中するため、複雑なタスクでも一貫性を保ちやすい特徴があります。
...
複数のエージェントが同じファイルを同時に編集しようとすると、何が起きるか。最後に書き込んだエージェントの変更が残り、他の変更は消える。あるいはマージの試みが失敗し、コードが破損する。並列化による速度向上の恩恵を受ける前に、この問題を解決しなければなりません。
...
フェーズ1（T1・T2・T3）は依存関係なしで並列実行できる。フェーズ2（T4・T5・T6）はフェーズ1の完了を待って順次開始する。依存関係のないタスクを先に並列実行し、後続タスクに向けてフェーズを分けることで、待機時間を最小化できます。
...
graph TD
    ...
    subgraph REVIEW["Review Layer"]
        PG_rev["Pragmatist Review"]
        SK_rev["Skeptic Review"]
        ID_rev["Idealist Review"]
        CN_rev["Connector Integration"]
    end
...
HNで報告された3倍速は「タスク並列実行」による速度向上です。一方、4性格体制は「品質並列検証」に主眼があります。

| 観点 | タスク並列（HN報告） | 品質並列（4性格体制） |
|------|---------------------|---------------------|
| 目的 | 処理時間の短縮 | レビュー品質の向上 |
| エージェントの役割 | 同質（同じタスクを分担） | 異質（異なる視点を担当） |
| 効果測定 | 実行時間（分） | 検出バグ数・指摘件数 |
| 適用場面 | 独立タスクが多い実装フェーズ | 設計レビュー・コードレビュー |

実運用では両方を組み合わせるのが効果的です。実装フェーズではworktreeを使ったタスク並列で速度を稼ぎ、レビューフェーズでは4性格体制で品質を担保します。
...
Agent Teamsのコスト構造を理解するうえで重要なのは、 **エージェント数に比例してトークン消費が増える** という点です。

筆者の運用経験に基づく概算です（correlate-workspace上で同程度の複雑さのコーディングタスクを複数回実行した際の平均値）。

| 構成 | 1タスクあたりのトークン消費（概算） | 倍率 |
|------|-----------------------------------|------|
| シングルエージェント | 基準（1x） | 1.0x |
| 2エージェント並列 | 約1.8x | 1.8x |
| 4エージェント並列 | 約3.2x | 3.2x |
| 4性格レビュー | 約2.5x | 2.5x |

4エージェント並列は理論上4倍のトークンを消費しますが、各エージェントのコンテキスト共有がない分、実測では3.2倍程度に収まる傾向があります。

### コスト対効果の判断基準

すべてのタスクに並列実行を適用すべきではありません。判断基準として以下を使っています。

- **並列化すべき**: 3つ以上の独立タスクがあり、合計所要時間が15分を超える場合
- **シングルで十分**: タスク間の依存関係が強い、または所要時間が10分未満の場合
- **4性格レビュー推奨**: セキュリティ関連の変更、APIの設計変更、本番環境への影響がある変更

Maxプラン（$200/月）で運用する場合、日常的にAgent Teamsを使っても上限に達することは稀です。ただし、5本以上の記事を同時並列生成するような高負荷ケースでは、1セッションで通常の3〜4倍のトークンを消費する点に注意が必要です。

---

## まとめ

HNで報告された「4エージェントで3倍速」は、特定の条件下で再現可能な数値であり、理論的にもアムダールの法則と整合する結果でした。

本記事で整理したポイントを振り返ります。

1. **3倍速の背景**: 50k LOCのコードベースで6タスクを4エージェント並列実行。逐次部分約11%が速度向上の上限を決めている
2. **ファイル競合の解決**: Git worktreesによるワークスペース分離が現時点で最も実践的
3. **品質の並列化**: 速度だけでなく、4性格体制のようなadversarial reviewによる品質向上も並列化の重要な活用法
4. **コスト**: トークン消費は約3.2倍。すべてのタスクに適用するのではなく、独立タスクが多い場面に絞るのが合理的

Agent Teamsは万能ではありません。しかし、タスク設計と排他制御を適切に行えば、ソロ開発者でも実用的な速度向上を得られるツールです。「3倍速は本当か」という問いには、「条件次第で本当だが、その条件を整えること自体がエンジニアリングだ」と答えるのが正確でしょう。
