...
Tailwind CSS v4は単なるマイナーアップデートではありません。設定ファイルの廃止、インポート記法の完全変更、PostCSSプラグインの分離など、根本的なアーキテクチャ変更が含まれています。特に `tailwind.config.js` への依存が深い shadcn/ui プロジェクトでは、何も準備せずに移行すると確実に詰まる。
