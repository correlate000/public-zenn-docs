...
:::message
本記事で紹介するのは、MCPプラグインを使わない連携方法です。Claude Codeのネイティブ機能（ファイル読み書き + CLAUDE.md参照）だけで、Obsidian Vaultへの自動蓄積を実現する。プラグイン設定のハードルなし、すぐに始められるアプローチ。
:::
...
1つ目はHandoffセクション。セッションの引き継ぎ情報を構造化して残す仕組みです。「完了状態」「未解決の課題」「次回最初にやること」「環境の状態」を明記する。次回セッション開始時、Claude CodeがこのHandoffを読むことで、文脈を失わずに作業を再開できます。

2つ目はErrors & Failuresセクション。エラーが発生したらセッション記録内に即座に記録し、同時に `05_errors/` に独立したエラーファイルを作成する。Obsidianのリンク `[[03_knowledge/errors/...]]` で双方向参照を実現しています。

3つ目はDecisions Madeテーブル。「何を、なぜ選んだか」をテーブル形式で記録します。後から「あの判断の根拠は何だったか」を検索できる。判断記録は `04_decisions/` にも独立ファイルとして保存されます。
...
ここで重要なのはチェックリスト。「CLAUDE.mdに反映済み」の項目がある。エラーの教訓をCLAUDE.mdに書き込むことで、次のセッション以降、Claude Codeが同じ失敗を予防してくれるようになるわけです。
...
Claude Codeとの連携で最も重要な設計判断が逐次書き込み。

:::message alert
Claude Codeのセッションは突然終了することがあります。トークン上限到達、ネットワーク切断、マシンのスリープ――理由はさまざま。セッション終了後にまとめて書き込む方式では、未保存の作業ログがすべて失われるリスクがある。
:::
