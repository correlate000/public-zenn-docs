...
一見不要に思えるかもしれない。しかし「夜勤シフト中心」の一行があるだけで、AIがスケジュール提案をする際に深夜帯を考慮するようになる。コンテキストは少ないほどいいという定説はあるが、判断に影響する属性情報は書くべきというのが実感。
...
コーディング規約（セクション7）について、Anthropic公式は「LLMにリンターの仕事をさせるな」と述べている[^2]。これは正しい。しかし、リンターでカバーできない規約は書く必要がある。
...
そしてFIRE基準（セクション13）。Financial Independence, Retire Earlyの考え方に基づく案件判断基準で、技術ブログでは珍しい項目。AIにビジネス判断の相談をする場面が増えた今、案件の受諾基準を明示しておくのは合理的な判断。
