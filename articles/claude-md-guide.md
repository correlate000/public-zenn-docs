...
Claude Codeを使い始めると、最初に出会うのがCLAUDE.mdファイル。`/init`コマンドで自動生成して、何となく追記して――そんな運用をしていないだろうか。

:::message
CLAUDE.mdは「設定ファイル」ではなく**AIエージェントとの契約書**。書き方ではなく「設計」の視点で捉え直すと、エージェントの挙動が劇的に安定する。
:::

私はワークスペース全体をカバーする15セクション構成のCLAUDE.mdを運用しています。本記事では、その設計思想と、複数プロジェクトを横断してきた中で見えてきたアンチパターン、さらにAgent Teams連携の基盤としてCLAUDE.mdを活用する方法まで解説する。

「とりあえず書く」から「意図を持って設計する」へ -- その転換点になれば幸いです。

## CLAUDE.mdとclaude.mdの違い -- メモリ階層を理解する

まず前提として、**CLAUDE.md（大文字）とclaude.md（小文字）はClaude Codeでは同一ファイルとして扱われる**。macOSのファイルシステム（APFS）がケース非感知のため、どちらの表記でも同じファイルとして認識される仕組み。私の環境では`~/dev/claude.md`（小文字）を使っているが、機能的にはCLAUDE.mdと同一。
...
起動時にClaude Codeは作業ディレクトリからルートまで再帰的にCLAUDE.mdを探索する。子ディレクトリのCLAUDE.mdはオンデマンド読み込み[^1]。そして**より具体的な指示が広い指示より優先される**仕組み。
...
私の環境では、Layer 2にあたる`~/dev/claude.md`が今回の主役。ワークスペース全体の「環境ガイド」として機能し、どのプロジェクトでClaude Codeを起動しても読み込まれるようになっている。

### AGENTS.mdとの関係

OpenAIが提唱するAGENTS.mdやGoogleのGEMINI.mdも同様の目的を持つファイルだが、Claude CodeはAGENTS.mdを読み込まない。ツールごとに専用のファイルが必要というのが現状の制約。
...
ここからが本記事の核心部分。実際に運用している`~/dev/claude.md`は15セクションで構成されている。「なぜこの構成にしたか」の設計判断を、カテゴリ別に解説します。

### セクション一覧と設計意図

15セクションは大きく4つのカテゴリに分類できる。
...
一見不要に思えるかもしれない。しかし「夜勤シフト中心」の一行があるだけで、AIがスケジュール提案をする際に深夜帯を考慮するようになる。コンテキストは少ないほどいいという定説はあるが、**判断に影響する属性情報は書くべき**というのが実感。

### WHERE -- 環境の全体像

フォルダ構成（セクション2）は最も長いセクションだが、省略できない。
...
AIエージェントはファイルシステムを「手探り」で探索する。この地図がないと、無関係なディレクトリを延々と`ls`し続ける非効率な動きをするわけです。
...
コーディング規約（セクション7）について、Anthropic公式は「LLMにリンターの仕事をさせるな」と述べている[^2]。これは正しい。しかし、**リンターでカバーできない規約**は書く必要がある。

[^2]: [Best practices for agentic coding](https://www.anthropic.com/engineering/claude-code-best-practices) -- Anthropic公式ブログ

例えば「日本語コメント可」という一行。これがないとAIは英語コメントで統一しようとする。`!important`の禁止もStylelintで検出可能だが、そもそも書かせない方が修正コストが低い。
...
セッション間のコンテキスト引き継ぎを仕組み化したもの。Claudeのコンテキストウィンドウは有限なので、次のセッションで「前回どこまでやったか」を確認できる仕組みが必要になる。

### WHAT NOT -- 制約と判断基準

セキュリティ（セクション10）の禁止事項はチェックリスト形式で記述する。
...
そして**FIRE基準**（セクション13）。Financial Independence, Retire Earlyの考え方に基づく案件判断基準で、技術ブログでは珍しい項目。AIにビジネス判断の相談をする場面が増えた今、案件の受諾基準を明示しておくのは合理的な判断。
...
15セクション構成を示したところで、逆に「こう書いてはいけない」パターンも整理する。複数プロジェクトでの運用から学んだ実例。
...
「インデントは2スペース」「セミコロンあり」といったルールは、ESLintやPrettierに任せるべき。CLAUDE.mdに書くと二重管理になり、設定が乖離するリスクが生まれる。

ただし前述の通り、**リンターでカバーできない規約**（日本語コメントの可否、BEM命名の採用方針など）はCLAUDE.mdの守備範囲。

### 3. 「キャッシュのせいにしない」が必要になった理由

実際に起きた事例。Claudeがバグの原因をブラウザキャッシュのせいにして、自分のコードを疑わなかった。

> 「ブラウザのキャッシュをクリアすれば直ります」

これを3回繰り返されて、結局コードにバグがあったというオチ。以降、プロジェクトのCLAUDE.mdに以下を追加した。
...
CLAUDE.mdの真価は、個人利用だけでなく**Agent Teamsの基盤**としても発揮される。
...
最後に、CLAUDE.mdのライフサイクルについて触れておきたい。
...
1. **Phase 1 -- 初期化**: `/init`で基本構造を生成。ここで終わる人が多い
2. **Phase 2 -- 成長**: 開発中に気づいたルールを追記していく。コミットメッセージの規約、特定ライブラリの使い方など
3. **Phase 3 -- インシデント対応**: Claudeがやらかした時こそ成長のチャンス。「キャッシュのせいにするな」はこのフェーズで生まれた
4. **Phase 4 -- 成熟**: 肥大化したCLAUDE.mdを`.claude/rules/`に分割。パス限定ルールで必要なときだけ読み込まれるように最適化する

### プロジェクト種別による詳細度の違い

複数プロジェクトを運用してわかったこと。プロジェクトの複雑さとCLAUDE.mdの詳細度は比例する。
...
静的サイトに15セクションは過剰。フルスタックアプリに1セクションでは不足。**プロジェクトに合わせて設計する**のが本記事の最大のメッセージ。
