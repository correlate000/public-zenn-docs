#!/usr/bin/env python3
"""
publish-queue-v2.py — 承認キュー方式の公開スクリプト

status: publish-ready の記事を1日1本、ランダム時刻に公開する。

設計思想:
  Zenn AI コンテンツガイドライン（2026-03-10）対応。
  「人が主体」を担保するため、公開承認は人間が行い、
  実行タイミングのみ自動化する。
...
使い方:
  ...
  python3 scripts/publish-queue-v2.py --force      # 時刻/休日チェックをスキップ
  python3 scripts/publish-queue-v2.py --list       # publish-ready 一覧表示
"""
import argparse
import hashlib
import json
...
def main():
    ...
    # 1本だけ選ぶ（リスト先頭 = アルファベット順で最初）
    slug, path = eligible[0]
